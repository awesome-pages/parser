...
import type { ParseResultFile, Artifact } from "./index";

export async function runParse(opts: NormalizedOptions): Promise<ParseResultFile[]> {
    ...
    for (const from of spec.from) {
      ...
      if (isLocal) {
        ...
        if (hasGlob) {
          ...
          const abs = path.isAbsolute(from) ? from : path.join(opts.rootDir, from);
          inputs = [abs];
        }
      } else {
        inputs = [from];
